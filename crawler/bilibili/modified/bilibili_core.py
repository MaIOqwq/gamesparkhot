...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if config.CRAWLER_TYPE == "search":
                ...
                # 每一小时执行一次增量爬取
                while True:
                    utils.logger.info("[BilibiliCrawler.start] Waiting for 1 hour before incremental crawl...")
    ...
    async def search_by_keywords(self):
        ...
        # 指定的keyword列表
        keywords = ["手机游戏", "王者荣耀", "和平精英", "原神", "崩坏星穹铁道", "绝区零", "明日方舟", "崩坏三", "天涯明月刀", "无限暖暖", "英雄联盟手游", "金铲铲之战", "明日方舟终末地", "三角洲行动", "火影忍者手游", "燕云十六声", "逆水寒手游", "永劫无间手游", "光遇", "第五人格", "阴阳师"]
        # 从第一页开始爬取
        start_page = 1
                ...
                if is_first_crawl:
                    ...
                    if need_crawl_default:
                        ...
                        result = await self._crawl_videos(
                            ...
                            min_timestamp_factor=0,  # 不限制时间（测试用）
                            max_timestamp_factor=30 * 24,  # 小于一个月
                            min_play_count=5000  # 播放量大于5000
                        )
