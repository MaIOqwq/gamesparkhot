...
class TrendPredictor:
    ...
    def feature_engineering(self, df: pd.DataFrame) -> tuple:
        ...
        # 生成三分类标签
        # 2 (上升)：change_rate > +0.1
        # 0 (下降)：change_rate < -0.1
        # 1 (平稳)：-0.1 ≤ change_rate ≤ 0.1
    ...
    def save_model(self, output_dir: str = '.'):
        ...
        # 保存模型
        model_path = os.path.join(output_dir, 'trend_classifier.json')
        self.model.save_model(model_path)
        logger.info(f"模型保存成功: {model_path}")

        # 保存特征列名
        feature_path = os.path.join(output_dir, 'trend_feature_columns.pkl')
        with open(feature_path, 'wb') as f:
            pickle.dump(self.feature_columns, f)
        logger.info(f"特征列名保存成功: {feature_path}")

        # 保存 LabelEncoder
        encoder_path = os.path.join(output_dir, 'trend_label_encoder.pkl')
        with open(encoder_path, 'wb') as f:
            pickle.dump(self.label_encoder, f)
        logger.info(f"LabelEncoder保存成功: {encoder_path}")

        # 保存类别映射字典
        class_map_path = os.path.join(output_dir, 'trend_class_map.pkl')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='XGBoost三分类模型训练脚本',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python train_xgboost_trend.py              # 默认配置文件和输出目录
  python train_xgboost_trend.py --config /path/to/config.ini  # 指定配置文件
  python train_xgboost_trend.py --output /path/to/output  # 指定输出目录
        """
    )
    parser.add_argument(
        '--config',
    ...
    logger.info("=" * 60)
    logger.info("XGBoost三分类模型训练脚本启动")
    logger.info("=" * 60)

    predictor = TrendPredictor(config_path=args.config)
