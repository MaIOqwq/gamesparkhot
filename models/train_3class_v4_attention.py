"""
3鍒嗙被瓒嬪娍棰勬祴 v4 - XGBoost + Attention 鍔犳潈鐗瑰緛
- 瀵硅繃鍘?12 涓獥鍙ｅ仛 attention 鍔犳潈锛岀敓鎴愭憳瑕佺壒寰?- 鏃堕棿鍒掑垎: 2020~2025.06 璁粌, 2025.07~2025.12 楠岃瘉, 2026~ 娴嬭瘯
"""
...
# ========= Attention 鍔犳潈鐗瑰緛 =========
# 瀵规牳蹇冪壒寰侊紝瀵硅繃鍘?12 涓獥鍙ｅ仛 attention 鍔犳潈鑱氬悎
print(f"[{datetime.now():%H:%M:%S}] 璁＄畻 Attention 鐗瑰緛...")
...
for feat in ATTN_FEATS:
    ...
    for i in range(1, ATTN_LAGS + 1):
        col = f'_{feat}_lag_{i}'
        agg[col] = agg.groupby('keyword')[feat].shift(i)
        lag_cols.append(col)
    ...
    for idx, row in agg.iterrows():
        current = row[feat]
        past_vals = row[lag_cols].values.astype(float)

        # 鐩镐技搴?= -|宸€紎^0.5 (寮€鏂规斁澶у皬骞呭彉鍖栫殑褰卞搷)
        # 鐢?-abs(diff) 浣垮€间笌褰撳墠瓒婃帴杩戠殑绐楀彛鏉冮噸瓒婂ぇ
        diff = np.abs(current - past_vals)
        scores = -diff

        # 澶勭悊 NaN (濡傛灉鏈夌獥鍙ｆ病鏈夎冻澶熷巻鍙?
        valid = ~np.isnan(scores)
        if valid.sum() == 0:
            attn_vals.append(0.0)
            continue

        # 灏嗘棤鏁堜綅缃殑鍒嗘暟璁句负 -inf锛宻oftmax 鍚庢潈閲嶄负 0
        scores[~valid] = -np.inf
        # softmax (鏁板€肩ǔ瀹?
        scores_shifted = scores - scores[valid].max()
        exp_s = np.exp(scores_shifted)
        weights = exp_s / exp_s.sum()

        # 鍔犳潈鑱氬悎 (NaN 鐨?past_vals 涔熶箻 0锛屼笉褰卞搷)
        past_vals_safe = np.nan_to_num(past_vals, 0)
        attn = np.sum(weights * past_vals_safe)
        attn_vals.append(attn)

    agg[f'attn_{feat}'] = attn_vals

print("  Attention 鐗瑰緛:", [f'attn_{f}' for f in ATTN_FEATS])
...
# 鐗瑰緛鍒?(鎺掗櫎 attention 鐢ㄧ殑涓棿 lag 鍒?
exclude = {'keyword', 'window_start', 'future_total_hot', 'change_rate', 'label'}
...
# ========= 鐗瑰緛閫夋嫨 =========
print(f"[{datetime.now():%H:%M:%S}] 鐗瑰緛閫夋嫨 (SelectKBest, k=60)...")
...
def evaluate(model, X, y, desc=""):
    ...
    print(f"  {desc}: acc={acc*100:.2f}%, extreme_acc={extreme*100:.2f}%")
    return acc
...
print(f"鐩爣: {TARGET_ACC*100:.0f}% {'OK' if best_acc >= TARGET_ACC else 'FAIL'}")
print(f"{'='*50}")
...
if hasattr(best_xgb, 'feature_importances_'):
    ...
    if attn_found:
        ...
        print(f"  Attention 鐗瑰緛骞冲潎鎺掑悕: {avg_rank:.0f}/{len(fi)}")
    else:
        print("  Attention 鐗瑰緛鏈閫夋嫨")
else:
    print("  鏃犳硶鑾峰彇鐗瑰緛閲嶈鎬?)
...
if best_name != 'ensemble':
    if hasattr(final_model, 'save_model'):
        final_model.save_model('trend_3class_v4_model.json')
        model_type = 'xgboost'
    else:
        final_model.booster_.save_model('trend_3class_v4_model.json')
        model_type = 'lightgbm'
else:
    best_xgb.save_model('trend_3class_v4_model.json')
    model_type = 'xgboost'
...
with open('trend_3class_v4_model_meta.json', 'w', encoding='utf-8') as f:
    json.dump(meta, f, ensure_ascii=False, indent=2)

print(f"\n妯″瀷宸蹭繚瀛? trend_3class_v4_model.json")
print(f"鍏冩暟鎹? trend_3class_v4_model_meta.json")
print(f"鏈€缁? {best_acc*100:.2f}% {'OK 杈炬爣' if best_acc >= TARGET_ACC else 'FAIL 鏈揪鏍?}")
