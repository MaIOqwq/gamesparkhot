...
class TrendPredictorBest:
    ...
    def train_model(self, X_train, y_train, X_val, y_val):
        ...
        logger.info(f"使用最佳参数: {best_params}")
    ...
    def save_model(self, output_dir: str = '.'):
        ...
        # 保存模型
        model_path = os.path.join(output_dir, 'trend_classifier_final.json')
        self.model.save_model(model_path)
        logger.info(f"最终模型保存成功: {model_path}")

        # 保存特征列名
        feature_path = os.path.join(output_dir, 'trend_feature_columns_final.pkl')
        with open(feature_path, 'wb') as f:
            pickle.dump(self.feature_columns, f)
        logger.info(f"特征列名保存成功: {feature_path}")

        # 保存 LabelEncoder
        encoder_path = os.path.join(output_dir, 'trend_label_encoder_final.pkl')
        with open(encoder_path, 'wb') as f:
            pickle.dump(self.label_encoder, f)
        logger.info(f"LabelEncoder保存成功: {encoder_path}")

        # 保存类别映射字典
        class_map_path = os.path.join(output_dir, 'trend_class_map_final.pkl')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='XGBoost三分类模型最终训练脚本',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python train_xgboost_trend_best.py              # 默认配置文件和输出目录
  python train_xgboost_trend_best.py --config /path/to/config.ini  # 指定配置文件
  python train_xgboost_trend_best.py --output /path/to/output  # 指定输出目录
        """
    )
    parser.add_argument(
        '--config',
    ...
    logger.info("=" * 60)
    logger.info("XGBoost三分类模型最终训练脚本启动")
    logger.info("=" * 60)

    predictor = TrendPredictorBest(config_path=args.config)
